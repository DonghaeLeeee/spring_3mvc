...
<head>
<meta charset="UTF-8">
<title>spring web 회원정보 수정</title>
...
<table>
	<tr>
		...
		<td id="join">
		<input type="text" id="lblId" name="id" value="${member.id }"readonly="readonly">
		</td>
	</tr>
	<tr>
		<td>
		...
		<td id="join">
		<input type="text" id="lblName" name="name" value="${member.name }" disabled>
		</td>
	</tr>
	<tr>
		<td>
	...
	<tr>
		...
		<td id="join">
		<input type="email" id="lblEmail" name="email" value="${member.email }">
		</td>
	</tr>
	<tr>
		<td>
		...
		<td id="join">
		<input type="number" id="lblAge" name="age" value="${member.age }">
		</td>
	</tr>
	<tr>
		<td>
...
document.querySelectorAll('#lblAddr option').forEach(ele => {
	if(ele.value=='${member.address}') ele.selected=true
})
document.querySelectorAll('input[name="hobby"]').forEach(ele => {
	if('${member.hobbies}'.includes(ele.value)) ele.checked=true
	else
		ele.checked=false
})
document.querySelectorAll('input[name="gender"]').forEach(ele => {
	if('${member.gender}'== ele.value) ele.checked=true
})
</script>
</body>
</html>